...
def check_watch(ctx):
    ...
    if len(message.role_mentions)==1:
        watch=message.role_mentions[0]
    else:
        watch=message.mentions[0]
    return watch

def load_settings():
    for setting in not_settings:
        Notifications(setting['server'],setting['watch'],setting['watch_channel'],setting['message'],setting['not_channel'])

def list_settings():
    types=['Pos.','Trigger Member/Role','Trigger Channel','Report Channel','Message']
    table_data=[types]
    for pos,setting in enumerate(Notifications.not_list):
        convert=[str(pos+1).strip(),setting.watch.name,setting.watch_channel.name,setting.not_channel.name,str(setting.message)]
...
async def notif(ctx,watch,watch_channel:discord.Channel,not_channel:discord.Channel,*,message:str='None'):
    ''': Add notification settings to the server'''
    server=ctx.message.server
    watch=check_watch(ctx)
    new_notification={
        'server':server.id,
        'watch':watch.id,
    ...
    if new_notification not in not_settings:
        ...
        Notifications(server.id,watch.id,watch_channel.id,message,not_channel.id)
        msg=list_settings()
